...
package cm.tbg.gpchat.activities.calling;
...
public class CallingActivity extends BaseActivity implements CallingService.StartFailedListener, ServiceConnection {
    ...
    private FloatingActionButton btnRejectAnswer;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (user != null) {
            ...
            if (user.getUserLocalPhoto() != null) {
                ...
                Glide.with(this).load(user.getThumbImg()).into(imgUser);
            }
            ...
            getDisposables().add(FireManager.fetchUserByUid(uid)
                    .subscribe(user -> {
                                if (user != null) {
        ...
        btnRejectAnswer.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                endCall();
            }
        ...
        if (user != null)
            getDisposables().add(getFireManager().checkAndDownloadUserPhoto(user).subscribe(imageItem -> {
                        if (imageItem != null && imageItem.getPhoto() != null)
                            Glide.with(CallingActivity.this).load(imageItem.getPhoto()).into(imgUser);
    ...
    private void hideOrShowButtons(boolean showHangup) {
        if (showHangup) {
            ...
            btnAnswer.hide();
            btnHangup.show();
            btnRejectAnswer.show();
        } else {
            btnReject.show();
            btnAnswer.show();
            btnHangup.hide();
            btnRejectAnswer.hide();
        }
    ...
    private void initViews() {
        ...
        btnRejectAnswer = findViewById(R.id.btn_reject_answer);
        ...
        localViewViewGroup = findViewById(R.id.local_view);
    }
...
//            //if the local view has no parent(not added to the layout yet) then add it
            if (localView.getParent() == null) {
                ...
                localViewViewGroup.addView(vc.getLocalView());
    ...
    private void setConstraintToBelowImageHolder(ConstraintSet set) {
        if (set == null) {
            ...
            set.clone(constraint);
            set.connect(localViewViewGroup.getId(), ConstraintSet.TOP, R.id.imageView3, ConstraintSet.BOTTOM);
            set.connect(localViewViewGroup.getId(), ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END);
            set.applyTo(constraint);
        } else {
            set.connect(localViewViewGroup.getId(), ConstraintSet.TOP, R.id.imageView3, ConstraintSet.BOTTOM);
            set.connect(localViewViewGroup.getId(), ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END);
    ...
    private void setLocalViewConstraintsToTop(ConstraintSet set) {
        set.connect(localViewViewGroup.getId(), ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP);
        set.connect(localViewViewGroup.getId(), ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END);
    }
    ...
    private void removeVideoViews() {
        ...
        if (vc != null) {
            ...
            if (vc.getLocalView().getParent() != null)
                localViewViewGroup.removeAllViews();
